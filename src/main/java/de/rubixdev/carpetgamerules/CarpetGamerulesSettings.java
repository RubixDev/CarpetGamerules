...
public class CarpetGamerulesSettings {
    ...
    @Rule(
        ...
        extra = "Whether the server should skip checking player speed when the player is wearing elytra. Often helps with jittering due to lag in multiplayer",
        category = { GAMERULE, PLAYER }
    )
    public static boolean disableElytraMovementCheck = false;

    @Rule(desc = "Disable raids", extra = "Whether raids are disabled", category = { GAMERULE, MOBS })
    ...
    @Rule(
        ...
        extra = "The maximum length of a chain of commands that can be executed during one tick. Applies to command blocks and functions",
    ...
    @Rule(desc = "Reduce debug info", extra = "Limits contents of debug screen", category = { GAMERULE, MISC })
    ...
    @Rule(
        ...
        extra = "The number of blocks outward from the world spawn coordinates that a player spawns in when first joining a server or when dying without a personal spawnpoint",
        category = { GAMERULE, PLAYER },
        strict = false,
        options = { "0", "1", "5", "10", "50" }
    )
    ...
    @Rule(
        ...
        extra = "Angered neutral mobs attack any nearby player, not just the player that angered them. Works best if `forgiveDeadPlayers` is disabled",
        category = { GAMERULE, MOBS }
    )
